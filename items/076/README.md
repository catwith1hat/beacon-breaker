...
- **T3: Modified `get_ancestor` callers.** Spec breaking change (returns ForkChoiceNode instead of Root). Audit all caller sites in each client for proper consumption of both fields. **CLOSED 2026-05-14 via item #79 with divergence: confirmed in nimbus.** Nimbus's main fork-choice (`proto_array.nim`, `fork_choice.nim`) has zero Gloas payload-status integration — no `is_supporting_vote`, `should_extend_payload`, `is_payload_data_available`, `is_payload_timely`, or `notify_ptc_messages`; `findHead` is pure phase-0 style. Per-block payload variants not tracked. SSZ types defined but no consumer in fork-choice. Other 5 clients have payload-status-aware fork-choice with structurally different APIs (root-returning ancestor vs ForkChoiceNode-returning ancestor); semantic byte-equivalence requires scenario testing.
- **T4: `is_supporting_vote` semantic.** New vote-classification function for fork-choice scoring. High-leverage for fork-choice integrity. **CLOSED 2026-05-14 via items #80 + #81 (no additional finding).** Branch A same-slot routing diverges in prysm/lodestar/grandine — see #80. `get_weight` previous-slot zeroing (consumer of `is_supporting_vote`) diverges in prysm/grandine — see #81. Branch B (ancestor accumulation) is structurally diverse across the 5 active clients (lodestar's intra-block PENDING-as-parent edge at `protoArray.ts:486`; prysm's dual-tree `n.weight = n.balance + en.weight + fn.weight` at `gloas.go:89`; teku's `BlockNodeVariants`; grandine's `AttestingBalances` propagation in `misc.rs:141-163`) but each appears semantically equivalent to spec on source review. Proposer-boost as synthetic vote with `payload_present=False` is correctly excluded from FULL/EMPTY of boost block in lodestar (`applyScoreChanges:367`) and accumulates correctly through ancestor variants. Byte-equivalence at scenario level requires simulator harness.
...
- **T6: `get_attestation_score` Gloas modification.** Spec adds payload-status considerations to attestation weight. Bug-likely. **CLOSED 2026-05-14 via item #80 with divergence: confirmed in prysm (asymmetric), lodestar, grandine.** Spec's `is_supporting_vote` returns False for FULL/EMPTY at `message.slot == block.slot`. Lighthouse and teku route same-slot votes to PENDING bucket only. Prysm asymmetrically routes same-slot FULL→FULL bucket while same-slot EMPTY→PENDING. Lodestar routes same-slot votes directly to FULL/EMPTY variant per payloadPresent (no slot check). Grandine accumulates same-slot votes into FULL/EMPTY buckets unconditionally. Same-slot voting is the common case for honest on-time attestations in Gloas.
- **T7: `get_weight` Gloas modification.** Same — payload-status now affects fork-choice weights. **CLOSED 2026-05-14 via item #81 with divergence: confirmed in prysm and grandine.** Spec zeros FULL/EMPTY weights at `block.slot + 1 == current_slot`. Lighthouse, teku, lodestar implement the zeroing. Prysm's `choosePayloadContent` compares raw `fn.weight`/`en.weight` at previous-slot, falling back to `shouldExtendPayload` only on weight ties. Grandine's segment-based `score()` lacks the previous-slot zeroing entirely. Cascades with #80: same-slot votes contribute to FULL/EMPTY weights, those weights then incorrectly affect previous-slot tiebreaks.
- **T8: `record_block_timeliness` Gloas modification.** New tracking dimensions for timely vs late blocks under ePBS. **CLOSED 2026-05-14 via item #82 with divergence: confirmed in prysm, teku, lodestar, grandine.** Spec records 2-tuple `[ATTESTATION_TIMELINESS_INDEX, PTC_TIMELINESS_INDEX]` used by `should_apply_proposer_boost` to suppress boost on early (PTC-timely) equivocation. Only lighthouse fully implements both tracking and suppression. Teku tracks 2-tuple but explicitly defers the suppression branch (`ForkChoiceUtilGloas.java:163-167` TODO). Prysm/lodestar use single-boolean timeliness with no equivocation suppression at all. Grandine has equivocation suppression but counts all siblings without PTC-timeliness filter — more strict than spec.
- **T9: Re-org timing.** ePBS introduces new re-org scenarios (EMPTY-block re-org, late-payload-envelope re-org). Per-client behavior at the boundary. **CLOSED 2026-05-14 via item #83 with divergence: confirmed in prysm and lodestar.** Spec Gloas `is_head_weak` adds equivocating-validator weight from head-slot committees to head_weight for monotonicity. Lighthouse (`proto_array.rs:677-678`), teku (`ForkChoiceUtilGloas.java:339-342`), and grandine (`store.rs:1192-1211`) implement the addition. Prysm (`gloas.go:507`, `reorg_late_blocks.go:82,154`) and lodestar (`forkChoice.ts:433`; spec link cites phase-0) use raw consensus-node weight without the monotonicity term.
- **T10: Threshold semantics on PTC vote ties.** Spec uses `>` (strict greater). Verify all 6 use strict greater (not `>=`). **CLOSED 2026-05-14 with no divergence.** All 5 active clients use strict `>`: prysm `gloas.go:258`, lighthouse `proto_array.rs:206`, teku `ForkChoiceModelGloas.java:450,458`, lodestar `protoArray.ts:689`, grandine `store.rs:929`. Nimbus has no Gloas fork-choice (#79). No new audit item needed.
